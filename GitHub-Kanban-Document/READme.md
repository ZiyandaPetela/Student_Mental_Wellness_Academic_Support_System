...
## README: Customization Justification

### Added Columns
1. **Clinical Review**  
   *Purpose*: Ensures mental health components (US-003, US-005) meet clinical standards before release  
   *Workflow Impact*: Creates dedicated QA stage for therapy team (@TherapyTeam)

2. **Compliance Check**  
   *Purpose*: Mandatory FERPA/HIPAA verification (US-002, US-013)  
   *Workflow Impact*: Final security gate before marking items "Done"

3. **Academic Review**  
   *Purpose*: Validates education features (US-006, US-010) with academic staff  
   *Workflow Impact*: Parallel review track for school-related functionality

### Task Assignment
- **US-001**: Assigned to @FrontEndDev (Registration implementation)
- **US-002**: Assigned to @SecurityLead (MFA development)
- **US-010**: Assigned to @AcademicCoord (Scheduling system)

### Labels Applied
- `must-have`: Critical path items (US-001, US-002, US-003)
- `technical`: Development tasks (US-001, US-014)
- `clinical`: Mental health features (US-003, US-005)
- `academic`: School-related items (US-006, US-010)

## Workflow Rules Configured
1. **Auto-advancement**: Issues move to "In Progress" when assigned
2. **WIP Limits**: 
   - "In Progress" column capped at 3 items
   - "Ready" column capped at 5 items
3. **Review Requirements**:
   - All clinical features require 2 approvals
   - Security items need security team sign-off (@SecurityTeam)
